...
[[nodiscard]] auto test_parse_toplevel(std::string_view src) -> std::string
{
  ...
  try {
    auto toplevel = parse_toplevel(src);
    return output + to_string(toplevel);
...
{
  SECTION("Single right paren")
  {
    verify_toplevel(")");
  }

  SECTION("Multiple toplevel")
  {
    verify_toplevel("1 3 4");
  }
}
...
{
  SECTION("Valid lambda")
  {
    verify_toplevel("(lambda (x y) (+ x y))");
  }
...
TEST_CASE("Parser let")
{
  SECTION("Valid let")
  {
    verify_toplevel("(let ((x 10) (y 20)) (+ x y))");
  }
}

TEST_CASE("Parse if expression")
{
  SECTION("Valid if expression")
  {
    verify_toplevel("(if (> x y) x y)");
  }

  SECTION("Missing right paren")
  {
    verify_toplevel("(if (> x y)");
  }

  SECTION("Missing else")
  {
    verify_toplevel("(if (> x y) 10");
  }

  SECTION("third branch")
  {
    verify_toplevel("(if (> x y) 10 20 30");
  }
}
...
TEST_CASE("Parse definition")
{
  ...
  {
    verify_toplevel("(define x 42)");
  }

  SECTION("Definition without an expression")
  {
    verify_toplevel("(define x)");
  }
